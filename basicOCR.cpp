...
void basicOCR::getData()
{
	...
	for(i =0; i<classes; i++){
		for( j = 0; j< train_samples; j++){
			...
			//Load file
			if(j<10)
				sprintf(file,"%s%d/%d0%d.jpg",file_path, i+1, i+1 , j);
			else
				sprintf(file,"%s%d/%d%d.jpg",file_path, i+1, i+1 , j);
			...
			//process file
			prs_image = cvCloneImage(preprocessing(src_image, size, size));
			
			//Set class label
			cvGetRow(trainClasses, &row, i*train_samples + j);
			cvSet(&row, cvRealScalar(i+1));
			//Set data 
...
float basicOCR::classify(IplImage* img, int showResult)
{
	...
	float result;
	//process file
	prs_image = cvCloneImage(preprocessing(img, size, size));
	if(showResult)
	{

//		cvNamedWindow("prs");
//		cvShowImage("prs", prs_image);
//		cvMoveWindow("prs",1366/5,100);		
//		cvWaitKey(30);
//		cvDestroyWindow("prs");
	}
