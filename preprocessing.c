...
void findX(IplImage* imgSrc,int* min, int* max){
	...
	int minFound=0;
	CvMat data;
	CvScalar maxVal=cvRealScalar((imgSrc->height*10/11) * 255);
	CvScalar val=cvRealScalar(0);
	//For each col sum, if sum > width*0 then we find the min 
...
void findY(IplImage* imgSrc,int* min, int* max){
	...
	int minFound=0;
	CvMat data;
	CvScalar maxVal=cvRealScalar((imgSrc->width*10/11) * 255);
	CvScalar val=cvRealScalar(0);
	//For each col sum, if sum > width*0 then we find the min 
	//then continue to end to search the max, if sum< width*255 then is new max
